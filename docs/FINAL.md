...
"이동 전략은 변경 가능성이 높으니 전략 패턴으로 설계해야지!"

이 판단 자체는 틀리지 않았습니다. 문제는 **해법의 수준**이었습니다.
...
- AlwaysMoveStrategy, NeverMoveStrategy, FixedNumberStrategy... 테스트를 위해 전략 클래스를 계속 만들어야 했고, 테스트 코드가 프로덕션 코드보다 더 복잡해지는 상황이 벌어졌습니다.
...
이동 전략은 "특정 조건에서 전진 여부를 반환"하는 단순한 **로직 문제**이지, 인스턴스별 상태를 관리해야 하는 **구조 문제**가 아니었습니다.
...
무엇을 테스트하느냐보다 **무엇을 테스트하지 않기로 결정했는가**가 오히려 더 중요한 판단이었습니다.

---

## 2. 설계 수준: 멘탈 모델과 코드의 일치

MoveStrategy를 고치면서 "이 코드를 처음 보는 사람이 바로 이해할 수 있을까?"라는 질문이 떠올랐습니다. 그리고 답은 아니었습니다.

머릿속에서는 "자동차 이름", "총 라운드 수", "자동차들"이 각각 명확한 개념인데, 코드에서는 `string`, `number`, `Car[]`일 뿐이었습니다.
멘탈 모델이 그대로 도메인에 반영되는 것이 가독성의 최선이라고 판단했습니다.
...
- Car **배열** 수준의 비즈니스 로직(중복 검증, 일괄 이동, 우승자 조회)이 Controller에 흩어져 있었습니다.
- 이 로직들의 공통점은 **개별 Car가 아니라 Car 컬렉션 전체에 대한 규칙**이라는 것입니다.
  중복 검증은 이름들을 모아야 가능하고, 우승자 조회는 전체 포지션을 비교해야 합니다.
- 유틸 함수로 빼는 방법도 있지만, 일급 컬렉션을 택한 이유는 **배열 자체를 외부에 노출하지 않기 위해서**입니다.
  `#cars`를 private으로 감추면, 외부에서 배열을 직접 조작(`push`, `splice` 등)할 수 없고 반드시 `moveAll`, `getWinnerNames` 같은 의도가 드러나는 메서드를 통해서만 접근하게 됩니다.
  규칙의 응집 + 배열 직접 접근 차단, 이 두 가지가 일급 컬렉션을 선택한 이유입니다.
...
코드를 처음 보는 사람도 이 4개 클래스만 보면 "아, 자동차 이름이 있고, 자동차가 있고, 자동차들을 모아서 경주하고, 라운드 수만큼 반복하는구나"를 바로 이해할 수 있습니다.

사람이 문제를 인식하는 구조와 코드의 구조가 일치하니, 별도의 설명 없이도 코드가 스스로 의도를 전달되었습니다.
도메인 모델링에서 멘탈 모델의 반영을 강조하는 이유도 결국 이것 — **"코드를 읽는 비용을 최소화하는 것"**이 설계의 가장 근본적인 목적이기 때문이라는 생각이 들었습니다.
...
MVC 패턴을 적용하면서 "Model은 View에 의존하면 안 된다", "Controller는 Model과 View를 중재한다", "입력 검증은 Model에서" 같은 규칙들을 따랐습니다.
그리고 이 규칙들을 잘 지키는 것 자체가 좋은 설계라고 생각했습니다.

컴퓨터과학에서 유명한 코코넛 이야기가 있습니다 — 원주민이 활주로 옆에서 코코넛 헤드폰을 쓰고 비행기를 부르는 의식을 하는데, "비행기가 올 때 항상 이렇게 했으니까."
형식은 따르는데 왜 그 형식이 필요한지를 모르는 거죠.
돌이켜보니 저도 비슷했습니다.
"Model은 View에 의존하면 안 돼" — 왜? "MVC 규칙이니까." 그건 이유가 아니었습니다.
...
이번 리팩토링에서 MoveStrategy를 클래스에서 함수로 바꾼 것도, 검증 책임을 MVC 규칙이 아니라 변경 축 기준으로 재배치한 것도, 결국 이 계층의 위에서부터 생각한 결과입니다.

이 깨달음을 기준으로 세 가지를 재설계했습니다.
...
이 둘을 같은 폴더에 두면, UI를 수정할 때마다 도메인 코드를 건드리게 될 리스크가 생깁니다.
...
이번 리팩토링의 핵심 판단 중 하나가 MoveStrategy를 함수로 줄인 것이었습니다.
...
현재 `() => boolean`이 향후 확장을 과도하게 제한하는 건 아닌지, 아니면 그때 가서 바꾸면 되는 수준인지 — 멘토님의 판단 기준이 궁금합니다.
