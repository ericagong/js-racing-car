...
> **패턴(MVC), 수치(커버리지), 구조(추상 클래스) — 전부 도구일 뿐인데, 목표처럼 쫓고 있었다는 것입니다.**
...
| **통합 테스트**        | playGame (게임 실행 → 결과 반환) | 객체 간 협력이 올바른지, 새 기능 추가 시 side effect 감지 |

```javascript
// RacingCarGame.test.js — 통합 테스트
...
- **불가능한 입력 테스트**: 입력 파싱 레이어가 항상 string → 적절한 타입으로 변환하므로, 도메인 객체에 잘못된 타입이 들어오는 경로 자체가 존재하지 않습니다.
...
이렇게 일급 컬렉션을 도입하면, 게임 실행 로직은 더욱 간결해집니다.
...
- "Model은 Controller와 View에 의존하면 안 된다"
- "View는 Model에만 의존하고, Controller에 의존해서는 안 된다"
- "Controller는 Model과 View에 의존해도 된다"

같은 규칙들을 반드시 구현해야한다고 집착했습니다.
형식을 목적으로 착각한 것입니다. 패턴이나 규칙의 형식만 따르면서, **왜 그렇게 하는지** 원리를 모르는 채 코드를 작성하는 Cargo Cult Programming처럼요.

MVC라는 형식을 지키면 좋은 코드가 된다고 믿고 있었습니다.
하지만 **분리의 본질은 "변경의 이유가 다른 코드를 나누는 것"**이었습니다. SRP, OCP, DIP 같은 원칙이 상위 철학이고,
MVC는 그 철학을 적용한 결과물 중 하나일 뿐입니다.
중요한 건 **왜 분리하느냐**이지, **어떤 패턴으로 분리하느냐**가 아닙니다.

아래처럼 계층구조를 정리해서, 우선순위를 반영해 리팩토링을 진행했습니다.
...
- MoveStrategy: 클래스 → 함수 (로직 문제를 구조로 풀지 않는다)
- 검증 책임: MVC 규칙이 아니라 변경 축 기준으로 재배치
- 모듈명: `Controller` → `RacingCarGame` (패턴의 역할이 아니라, 모듈이 하는 일을 이름으로)

이 기준으로 세 가지 방향에서 리팩토링을 수행했습니다.
...
| **RacingCarGame** | 파싱/변환 (`"1,2,3"` → `[1,2,3]`) | 입력 형식의 변환. 도메인 규칙이 아님             |
...
### 3. 통합 테스트를 위한 export가 적절한가

- 기존에는 `playGame`이 private 함수였고, 통합 테스트에서 이를 복제하여 사용하고 있었습니다.
- 리팩토링 과정에서 **통합 테스트의 대상이 `playGame`이어야 한다**는 판단 하에, `moveStrategy`를 주입 가능하도록 변경하고 export했습니다.
...
- 이렇게 하면 mock 없이 `alwaysMove`, `neverMove` 같은 테스트 전략을 직접 주입하여 도메인 객체 간 협력을 검증할 수 있습니다.
- 하지만 **테스트를 위해 접근 제어를 완화하는 것**이 적절한지 고민이 됩니다.
    - 테스트 용이성을 위해 설계를 변경하는 것이 자연스러운 것인지?
    - 아니면 private을 유지한 채 mock 기반으로 `initializeApp()`을 통째로 테스트하는 것이 더 나은지?
    - 또는 `playGame`의 export가 오히려 해당 함수가 독립적인 모듈로 분리되어야 한다는 시그널인지?
