...
[V] 자동차 이름은 `5자 이하`만 가능하다.

[V] 사용자가 잘못된 입력 값(빈 값)을 작성한 경우 프로그램을 종료한다.
...
-   사용자 입력이 잘못된 경우
    ...
    [V] 사용자 입력이 빈 값인 경우, 오류를 발생시킨다.

-   자동차 이름이 잘못된 경우

    [V] 자동차 이름 중 중복된 이름이 있는 경우, 오류를 발생시킨다.

    [V] 자동차 이름 중 빈 값이 있는 경우, 오류를 발생시킨다.
...
[V] 코드 스타일 가이드에 따라 컨벤션을 준수하며 개발한다.

[V] 변수 선언시 var를 사용하지 않는다. let, const를 사용한다.

[V] 전역 변수를 만들지 않는다.

[V] 축약하지 않는다.

[V] 하드 코딩된 값 대신에 의미 있는 상수를 활용한다.

[V] 동등 연산자는 === 로만 사용한다.

[V] 함수(또는 메서드)의 길이가 10라인을 넘어가지 않도록 구현한다.

[V] 함수(또는 메서드)가 한 가지 일만 하도록 만든다.

[V] 함수(또는 메서드)의 들여쓰기 depth는 2단계까지만 허용한다.
...
[V] 사용자가 잘못된 입력 값을 작성한 경우 에러 메시지를 보여주고, 다시
입력할 수 있게 한다.
